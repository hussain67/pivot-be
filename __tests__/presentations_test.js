...
test("Get welcome message", async () => {
  const response = await request(app).get("/api/v1/presentations/welcome").set("Authorization", `Bearer ${presenterOne.tokens[0].token}`).expect(200);
});

describe("Create a presentation", () => {
  test("Create a presentation with proper given field", async () => {
    const response = await request(app).post("/api/v1/presentations").set("Authorization", `Bearer ${presenterOne.tokens[0].token}`).send({ title: "Creation of universe-2", slides: [] }).expect(201);
  ...
  test("Should not create a presentation without proper given field", async () => {
    const response = await request(app).post("/api/v1/presentations").set("Authorization", `Bearer ${presenterOne.tokens[0].token}`).send({}).expect(400);
    const presentation = await Presentation.findById(response.body._id);
...
describe("Get presentation by id", () => {
  test("Should fetch presentation by its id ", async () => {
    const response = await request(app).get(`/api/v1/presentations/${presentationOne._id}`).set("Authorization", `Bearer ${presenterOne.tokens[0].token}`).send().expect(200);

    expect(response.body.title).toBe("Chemical reaction 1");
  });

  test("Should not fetch presentation of other user", async () => {
    const response = await request(app).get(`/api/v1/presentations/${presentationOne._id}`).set("Authorization", `Bearer ${presenterTwo.tokens[0].token}`).send().expect(404);
...
describe("Get presentations by a prasenter", () => {
  test("Should fetch all the presentations by a presenter", async () => {
    const presentations = await request(app).get(`/api/v1/presentations`).set("Authorization", `Bearer ${presenterTwo.tokens[0].token}`).send().expect(200);
    expect(presentations.body.length).toBe(2);
  });

  test("Should return error message if no presentations found", async () => {
    const presentations = await request(app).get(`/api/v1/presentations`).set("Authorization", `Bearer ${presenterThree.tokens[0].token}`).send().expect(404);
...
describe("Delete presentation", () => {
  test("Should delete a presentation of an authenticated user", async () => {
    await request(app).delete(`/api/v1/presentations/${presentationTwo._id}`).set("Authorization", `Bearer ${presenterTwo.tokens[0].token}`).send().expect(200);
    const presentation = await Presentation.findById(presentationTwo._id);
    expect(presentation).toBeNull();
  });
  test("Should send error message for invalid id", async () => {
    const response = await request(app).delete(`/api/v1/presentations/"627da9794e7b9cbfd2351345h"`).set("Authorization", `Bearer ${presenterTwo.tokens[0].token}`).send().expect(404);
...
describe("Update a presentation", () => {
  test("Should update a presentation field", async () => {
    const presentation = await request(app).patch(`/api/v1/presentations/${presentationTwo._id}`).set("Authorization", `Bearer ${presenterTwo.tokens[0].token}`).send({ title: "updated title" }).expect(200);
    expect(presentation.body.title).toBe("updated title");
  });
  test("Should not update a presentation field of another presenter", async () => {
    await request(app).patch(`/api/v1/presentations/${presentationOne._id}`).set("Authorization", `Bearer ${presenterTwo.tokens[0].token}`).send({ title: "updated title" }).expect(404);
...
describe("Slides as a subdocument in a presentations", () => {
  test("Create slide", async () => {
    const response = await request(app).post(`/api/v1/presentations/${presentationOne._id}/slides`).set("Authorization", `Bearer ${presenterOne.tokens[0].token}`).send({ slideTitle: "slide two" }).expect(201);
    expect(response.body.slides.length).toBe(2);
  });
  test("Get all slides", async () => {
    await request(app).post(`/api/v1/presentations/${presentationOne._id}/slides`).set("Authorization", `Bearer ${presenterOne.tokens[0].token}`).send({ slideTitle: "slide two" });
    const response = await request(app).get(`/api/v1/presentations/${presentationOne._id}/slides`).set("Authorization", `Bearer ${presenterOne.tokens[0].token}`).send({}).expect(200);
